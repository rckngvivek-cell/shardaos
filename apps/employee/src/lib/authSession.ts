...
export function resendPlatformLoginOtp(input: ResendLoginOtpInput): Promise<LoginOtpChallenge> {
...
export async function restorePlatformSession(): Promise<AuthSession | null> {
  ...
  try {
    ...
    if (response.status !== 401) {
      throw new AuthHttpError('Unable to restore employee session', response.status);
    }
  } catch (error) {
