...
1. Root `firebase.json` defines the Firestore emulator plus the Emulator UI.
...
3. `GET /api/students`
4. `POST /api/students`
5. School portal shell with dashboard navigation and a students page that can call the API
...
5. Keep founder-only functionality out of the dedicated public portals.
...
3. The school app loads a dashboard shell and can render student data from the API.
...
3. Add the Attendance page in the school app with a day/class filter and submit flow.
